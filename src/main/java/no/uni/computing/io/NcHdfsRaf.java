...
//allow the java NetCDF library to read from HDFS
public class NcHdfsRaf extends RandomAccessFile {
  ...
  public NcHdfsRaf(FileStatus fileStatus, Configuration conf, int bufferSize)
                   throws IOException {
    ...
    this._fs = path.getFileSystem(conf);
   ...
   * @param conf a Configuration object containing information about the
   * currently executing job
   */
  public NcHdfsRaf(FileStatus fileStatus, Configuration conf)
                   throws IOException {

    this(fileStatus, conf, defaultBufferSize);
  }
